  ...
  <div class="stream">
    ...
    <div class="series" v-if="isSeries">
      <Segments :segments="seasons" v-model="selected.season">
        <template #segment="{ segment }">
          <span v-if="segment > 0"
            >{{ $t(`views.stream.season`) }} {{ segment }}</span
          >
    ...
    <div class="streams">
      <Title
        ...
        icon="play-outline"
        type="secondary"
        translate="views.stream.streams.title"
      />
      ...
      >
        ...
        <template #right="{ item }">
          <div class="info">
            ...
            <div class="title" :class="{ extend: !item.name }">
              {{ item.title }}
            </div>
          </div>
        </template>
...
export default {
  ...
  computed: {
    ...
    filterVideos() {
      return this.meta.videos
        .filter(({ season }) => season === this.selected.season)
  ...
  methods: {
    async loadStreams() {
      ...
      const installedAddons = this.collection.streams.filter((addon) =>
        this.installed.includes(addon.manifest.id)
      );
    ...
    parseSeason() {
      this.seasons = Array.from(
        new Set(
          this.meta.videos.map(({ season }) => season).sort((a, b) => a - b)
        )
      );
  ...
  async mounted() {
    ...
    if (this.meta.videos && this.meta.videos.length) {
      const episode =
        this.meta.videos.find(({ id: imdb_id }) => imdb_id === id) ||
        this.meta.videos[0];
