...
const AddonService = {
    ...
    createCollection(addons) {
        ...
        const filter = (manifest, resource) => {
            ...
            return id !== 'community.peario' && (idPrefixes && idPrefixes.includes('tt') && resources && resources.includes(resource) && (types.includes('movie') || types.includes('series')) || resources.find(({ name, idPrefixes, types }) => name === resource && idPrefixes && idPrefixes.includes('tt') && types && (types.includes('movie') || types.includes('series'))));
        };

        const streams = col.getAddons().filter(({ manifest }) => filter(manifest, 'stream'));
